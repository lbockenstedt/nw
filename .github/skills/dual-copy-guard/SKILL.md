...
> **Canonical source.** The authoritative pair list + compare commands live at
> `lm/.claude/skills/dual-copy-guard/reference.md`. This file is the
> Copilot-discoverable entry point (Copilot only scans `.github/skills/` and
> `~/.copilot/skills/`). **Before auditing, `Read` the canonical
> `lm/.claude/skills/dual-copy-guard/reference.md`** for the exact pair list,
> sync rules, and compare commands.
...
1. Read the canonical `reference.md` for the authoritative pair list.
...
1. Read the canonical `reference.md`.
2. Run each pair's compare command. Collect every mismatch.
3. For each mismatch, determine whether it's real drift or an expected
   difference (functional-parity pairs legitimately differ — judge intent, not
   bytes).
4. Report a table: pair, drift? (yes/expected), what's out of sync, fix.
5. Only auto-fix byte-identical pairs (`cp` canonical→generated). For
   functional-parity/twin pairs, propose the edit and confirm first.

## Non-negotiable rules

- **Never edit a generated copy directly.** Edit the canonical source and
  re-generate. The reference marks which is canonical.
- **Functional-parity ≠ byte-identical.** `common.sh`↔`common.ps1`, the two
  ...
  Sync the *behavior and shared data* (key lists, field arrays, thresholds,
...
- **Self-verifying.** Don't trust line numbers — read the current files to
  locate the equivalent construct.

## Scope note

T3 (`cs/clients/t3/`) is NOT a per-sim client — it's a virtual-WiFi/IoT emulator.
It shares only *delivery/config* plumbing, never the traffic sims. Treat it as a
config/delivery node in the audit, not a sim-parity target.
