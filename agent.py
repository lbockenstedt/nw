        ...
        async with websockets.connect(
            ...
            try:
                async for message in websocket:
                    ...
                    if cmd_type == "UPDATE_CONFIG":
                    ...
                    elif cmd_type == "PXMX_DRIVE_HEALTH":
                           # Drive health monitoring: query SSD wear levels via smartctl
                           # using cciss device interface. Reports wear levels (0-100%),
                           # model/serial info, health status, and historical trends.
                        try:
                            from drive_health import drive_health
                            result = await asyncio.to_thread(
                                drive_health.get_drive_health_for_ui
                             )
                            result["status"] = "SUCCESS"
                        except Exception as e:
                        ...
                        try:
                            from drive_health import drive_health
                            result = drive_health.install_ssacli_if_needed()
