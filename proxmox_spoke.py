...
# commit). The Agent's old typed handlers stay as a rollback fallback. Moved to
# the shared lm/core/src (alongside base_spoke) so a cs spoke hosting its own
# Proxmox agent can answer GET_NODE_STATS/PXMX_LIST_VMS too — see
# pxmx_node_vm_aggregation below.
try:
    import pve_cmd_builder  # noqa: E402
except ImportError:
    from core.src import pve_cmd_builder  # type: ignore  # noqa: E402

# Shared node-stats / VM-list aggregation (also used by cs spokes hosting
# their own Proxmox agent) — see core/src/messaging/pxmx_node_vm_aggregation.py.
try:
    from core.src.messaging import pxmx_node_vm_aggregation as _pxmx_agg
except ImportError:
    from messaging import pxmx_node_vm_aggregation as _pxmx_agg  # type: ignore  # noqa: E402
...
class ProxmoxSpoke(BaseSpoke):
    ...
    async def handle_command(self, command_type: str, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if cmd == "PXMX_DRIVE_HEALTH":
              ...
              # Routed to the agent which runs the health check and returns
              # drive wear levels, health status, and historical trends.
            agent_id = data.get("agent_id")
            if not agent_id:
                agent_id = self._agent_for_node(data.get("node", ""))
            if not agent_id:
            ...
            try:
                ...
                # Extract payload data if wrapped
        ...
        if cmd == "PXMX_INSTALL_SSACLI":
              ...
              # Routed to the agent which runs the installation and returns status.
            agent_id = data.get("agent_id")
            if not agent_id:
                agent_id = self._agent_for_node(data.get("node", ""))
            if not agent_id:
    ...
    # ── Node stats / VM list ──────────────────────────────────────────────────
    # The actual logic now lives in the shared core/src/messaging/
    # pxmx_node_vm_aggregation module — the same code a cs spoke hosting its
    # own Proxmox agent (split-topology) uses to answer these commands, so
    # both host kinds behave identically. BaseControlPlane.handle_system_command
    # answers GET_NODE_STATS/PXMX_LIST_VMS directly (mirroring GET_AGENTS)
    # BEFORE any command reaches here, so the handle_command branches above
    # for those two commands are effectively unreachable in normal operation —
    # these methods stay as thin delegating wrappers purely so the OTHER
    # internal callers below (_search_vms, _all_vms_info, _single_vm_info)
